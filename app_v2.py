...
# Configuration secrète (non affichée dans l'interface)
DEFAULT_BODY = {
    "data": {
        "influencer_id": 1564,
        "category_id": 26
...
# API URL input (optionnel si vous voulez le cacher aussi)
# api_url = st.sidebar.text_input("API URL", API_URL)
# ou utiliser directement la constante :
api_url = API_URL

# Utiliser le body par défaut (caché)
body = DEFAULT_BODY
...
- Body request: Pré-configuré (caché)
- API: Digital Creator Awards
- Mode: Multi-threadé
""")
...
if st.session_state.running:
    ...
    while st.session_state.running:
        try:
            ...
            batch_placeholder.info(f"**Batch {st.session_state.batch_count}:** ✅ {success_count} réussites | ❌ {failure_count} échecs")
...
st.sidebar.subheader("Paramètres actuels")
st.sidebar.write(f"**Workers:** {MAX_WORKERS}")
st.sidebar.write(f"**Taille des batches:** {BATCH_SIZE}")
st.sidebar.write(f"**Délai des requêtes:** {REQUEST_DELAY*1000}ms")
st.sidebar.write(f"**Délai des batches:** {BATCH_DELAY}s")
